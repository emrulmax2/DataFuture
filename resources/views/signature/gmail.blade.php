  ...
  <tr>
    ...
    @if($showPhoto)
      <td width="218" style="width:218px; padding:0; vertical-align:top">
        <img src="{{ $sig['photo_url'] }}" width="218" height="320" alt="{{ $sig['display_name'] }}" style="display:block; border:0">
      </td>
              ...
              @foreach($rows as $item)
                <tr>
                  <td width="19" style="width:19px; padding:0 13px {{ $loop->last ? '0' : '11px' }} 0; vertical-align:top">
                    <img src="{{ $icon($item[0]) }}" width="19" height="19" alt="{{ $item[1] }}" style="display:block; border:0; margin-top:1px">
                  </td>
      ...
      <table cellpadding="0" cellspacing="0" border="0" align="right" style="border-collapse:collapse">
        <tr>
          <td align="right" style="text-align:right">
            <a href="{{ $sig['website_url'] }}" style="text-decoration:none"><img src="{{ $icon('lcc-logo') }}" width="156" height="74" alt="{{ config('emailsignature.organisation') }}" style="display:block; border:0"></a>
          </td>
        </tr>
              ...
              <table cellpadding="0" cellspacing="0" border="0" align="right" style="border-collapse:collapse">
                <tr>
                  @foreach($sig['socials'] as $network => $url)
                    ...
                    <td style="padding-left:{{ $loop->first ? '0' : '9' }}px"><a href="{{ $url }}" style="text-decoration:none"><img src="{{ $icon($network) }}" width="22" height="22" alt="{{ ucfirst($network) }}" style="display:block; border:0"></a></td>
                  @endforeach
                </tr>
              </table>
            </td>
