    ...
    <div class="h-full flex items-center">
        ...
        <div class="intro-x dropdown w-8 h-8">
            <div class="dropdown-toggle w-8 h-8 rounded-full overflow-hidden shadow-lg image-fit zoom-in scale-110" role="button" aria-expanded="false" data-tw-toggle="dropdown">
                <img alt="{{ Auth::user()->load('employee')->employee->title->name.' '.Auth::user()->load('employee')->employee->first_name.' '.Auth::user()->load('employee')->employee->last_name }}"  src="{{ (isset(Auth::user()->load('employee')->employee->photo) && !empty(Auth::user()->load('employee')->employee->photo) && Storage::disk('google')->exists('public/employees/'.Auth::user()->load('employee')->employee->id.'/'.Auth::user()->load('employee')->employee->photo) ? Storage::disk('google')->url('public/employees/'.Auth::user()->load('employee')->employee->id.'/'.Auth::user()->load('employee')->employee->photo) : asset('build/assets/images/avater.png')) }}">
            </div>
            <div class="dropdown-menu w-56">
                <ul class="dropdown-content bg-primary/80 before:block before:absolute before:bg-black before:inset-0 before:rounded-md before:z-[-1] text-white">
                    <li class="p-2">
                        <div class="font-medium">{{ Auth::user()->load('employee')->employee->title->name.' '.Auth::user()->load('employee')->employee->first_name.' '.Auth::user()->load('employee')->employee->last_name }}</div>
