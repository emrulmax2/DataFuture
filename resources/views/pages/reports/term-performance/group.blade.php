    ...
    <div class="intro-y box p-5 mt-5">
        @if($result->count() > 0)
            ...
            <div class="overflow-x-auto scrollbar-hidden mt-5" id="attendanceRateWrap">
                ...
                <table class="table table-bordered table-sm" id="attendanceRateOvTable" data-title="{{ $term->name.' - '.$course->name }}">
                    <tbody>
                        <tr class="rateRow" data-label="{{ $group->name }}" data-rate="{{ ($overAll > 0 ? $overAll : 0) }}" data-bg="{{ $bgs[0] }}" data-bd="{{ $bds[0] }}">
                            <td class="w-20">
                        ...
                        @foreach($result as $res)
                            ...
                            @endphp
                            <tr class="rateRow" data-label="{{ $res->module_name }}" data-rate="{{ ($res->percentage_withexcuse > 0 ? round($res->percentage_withexcuse, 2) : 0) }}" data-bg="{{ $bgs[$randKey] }}" data-bd="{{ $bds[$randKey] }}">
                                <td class="w-20">
                                    <div class="form-check m-0 justify-center">
                                        <input checked id="rateRowCheck_{{ $row }}" class="form-check-input rateRowCheck" type="checkbox" name="rateRowCheck[]" value="1">
                                    </div>
                                </td>    
                                <th><a href="javascript:void(0);">{{ $res->module_name }}</a></th>
...
@section('script')
    @vite('resources/js/term-performance-group-reports.js')
@endsection