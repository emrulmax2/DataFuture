<div class="intro-y mt-5 flex justify-start">
    <a href="javascript::void(0)" class="btn shadow-lg border-0 bg-white mr-3 inline-block p-5 text-left w-54 active-bg-success active-text-white hover-bg-success hover-text-white">
        <span class="block text-lg text-dark font-semibold">Information</span>
        <span class="block text-base font-normal text-slate-500">Details</span>
    </a>
</div>