...
var employeeAppraisalNoteListTable = (function () {
    var _tableGen = function () {
        // Setup Tabulator
        let employeeId = $("#employeeAppraisalNoteListTable").attr('data-employee') != "" ? $("#employeeAppraisalNoteListTable").attr('data-employee') : "0";
        let appraisalId = $("#employeeAppraisalNoteListTable").attr('data-appraisal') != "" ? $("#employeeAppraisalNoteListTable").attr('data-appraisal') : "0";
        let queryStr = $("#queryNote").val() != "" ? $("#queryNote").val() : "";
        let status = $("statusNote").val() != "" ? $("#statusNote").val() : "1";

        let tableContent = new Tabulator("#employeeAppraisalNoteListTable", {
                ...
                },
                {
                    ...
                    formatter(cell, formatterParams) {                        
                        ...
                        if (cell.getData().deleted_at == null) {
                            btns += '<button data-id="' + cell.getData().id + '" data-tw-toggle="modal" data-tw-target="#viewAppraisalNoteModal"  class="view_btn btn btn-twitter text-white btn-rounded ml-1 p-0 w-9 h-9"><i data-lucide="presentation" class="w-4 h-4"></i></button>';
                            btns += '<button data-id="' + cell.getData().id + '" data-tw-toggle="modal" data-tw-target="#editAppraisalNoteModal" type="button" class="edit_btn btn-rounded btn btn-success text-white p-0 w-9 h-9 ml-1"><i data-lucide="Pencil" class="w-4 h-4"></i></a>';
        ...
        // Export
        $("#tabulator-export-csv-Note").on("click", function (event) {
            tableContent.download("csv", "data.csv");
        });

        $("#tabulator-export-json-Note").on("click", function (event) {
            tableContent.download("json", "data.json");
        });

        $("#tabulator-export-xlsx-Note").on("click", function (event) {
            window.XLSX = xlsx;
        ...
        $("#tabulator-export-html-Note").on("click", function (event) {
        ...
        // Print
        $("#tabulator-print-Note").on("click", function (event) {
            tableContent.print();
        });
    };
    return {
...
(function(){
    if ($("#employeeAppraisalNoteListTable").length) {
        ...
        // On click go button
        $("#tabulator-html-filter-go-Note").on("click", function (event) {
            filterHTMLFormEN();
        });

        // On reset filter form
        $("#tabulator-html-filter-reset-Note").on("click", function (event) {
            $("#queryNote").val("");
            $("#statusNote").val("1");
            filterHTMLFormEN();
        });
    ...
    addNoteModalEl.addEventListener('hide.tw.modal', function(event) {
        $('#addAppraisalNoteModal .acc__input-error').html('');
        $('#addAppraisalNoteModal input[name="document"]').val('');
        $('#addAppraisalNoteModal #addEmpNoteDocument').html('');
    ...
    editNoteModalEl.addEventListener('hide.tw.modal', function(event) {
        $('#editAppraisalNoteModal .acc__input-error').html('');
        $('#editAppraisalNoteModal input[name="opening_date"]').val('');
        $('#editAppraisalNoteModal input[name="document"]').val('');
        $('#editAppraisalNoteModal #editEmpNoteDocument').html('');
        $('#editAppraisalNoteModal input[name="id"]').val('0');
        $('#editAppraisalNoteModal .downloadExistAttachment').attr('href', '#').fadeOut();
    ...
    viewNoteModalEl.addEventListener('hide.tw.modal', function(event) {
        ...
        $('#viewAppraisalNoteModal .modal-footer .footerBtns').html('');
    });
    ...
    $('#addAppraisalNoteForm').on('change', '#addEmpNoteDocument', function(){
        showFileName('addEmpNoteDocument', 'addEmpNoteDocumentName');
    });
    
    $('#editAppraisalNoteForm').on('change', '#editEmpNoteDocument', function(){
    ...
    $('#employeeAppraisalNoteListTable').on('click', '.view_btn', function(e){
        var $btn = $(this);
        ...
        axios({
            ...
            $('#viewAppraisalNoteModal .modal-body').html(response.data.message);
            if(response.data.btns != ''){
                $('#viewAppraisalNoteModal .modal-footer .footerBtns').html(response.data.btns);
            }
            createIcons({
                icons,
                "stroke-width": 1.5,
    ...
    $('#addAppraisalNoteForm').on('submit', function(e){
        ...
        form_data.append('file', $('#addAppraisalNoteForm input[name="document"]')[0].files[0]); 
            ...
            if (error.response) {
                if (error.response.status == 422) {
                    for (const [key, val] of Object.entries(error.response.data.errors)) {
                        $(`#addAppraisalNoteForm .${key}`).addClass('border-danger');
                        $(`#addAppraisalNoteForm  .error-${key}`).html(val);
                    }
                } else {
                    console.log('error');
                }
    ...
    $('#employeeAppraisalNoteListTable').on('click', '.edit_btn', function(e){
        var $btn = $(this);
        var noteId = $btn.attr('data-id');
        axios({
            method: "post",
            ...
            $('#editAppraisalNoteModal [name="opening_date"]').val(dataset.opening_date ? dataset.opening_date : '');
            $('#editAppraisalNoteModal input[name="id"]').val(noteId);
            if(dataset.docURL != ''){
                $('#editAppraisalNoteModal .downloadExistAttachment').attr('href', dataset.docURL).fadeIn();
            }else{
                $('#editAppraisalNoteModal .downloadExistAttachment').attr('href', '#').fadeOut();
            }
        }).catch(error => {
    ...
    $('#editAppraisalNoteForm').on('submit', function(e){
        ...
        let form_data = new FormData(form);
        form_data.append('file', $('#editAppraisalNoteForm input[name="document"]')[0].files[0]); 
        ...
        axios({
            ...
            if (error.response) {
                if (error.response.status == 422) {
                    for (const [key, val] of Object.entries(error.response.data.errors)) {
                        $(`#editAppraisalNoteForm .${key}`).addClass('border-danger');
                        $(`#editAppraisalNoteForm  .error-${key}`).html(val);
                    }
                } else {
                    console.log('error');
                }
