                ...
                },
                {
                    ...
                    formatter(cell, formatterParams) {                        
                        ...
                        if (cell.getData().deleted_at == null) {
                            btns +='<a href="'+route('hr.bank.holiday', cell.getData().id)+'" class="btn-rounded btn btn-linkedin text-white p-0 w-9 h-9 ml-1"><i data-lucide="landmark" class="w-4 h-4"></i></a>';
                            btns +='<a href="'+route('holiday.year.leave.option', cell.getData().id)+'" class="btn-rounded btn btn-facebook text-white p-0 w-9 h-9 ml-1"><i data-lucide="list-ordered" class="w-4 h-4"></i></a>';
                            btns += '<button data-id="'+cell.getData().id +'" data-tw-toggle="modal" data-tw-target="#editHolidayYearModal" type="button" class="edit_btn btn-rounded btn btn-success text-white p-0 w-9 h-9 ml-1"><i data-lucide="Pencil" class="w-4 h-4"></i></a>';
...
(function () {
    ...
    const addHolidayYearModal = tailwind.Modal.getOrCreateInstance(document.querySelector("#addHolidayYearModal"));
    const editHolidayYearModal = tailwind.Modal.getOrCreateInstance(document.querySelector("#editHolidayYearModal"));
    const successModal = tailwind.Modal.getOrCreateInstance(document.querySelector("#successModal"));
    const confirmModal = tailwind.Modal.getOrCreateInstance(document.querySelector("#confirmModal"));

    let confModalDelTitle = 'Are you sure?';

    const addHolidayYearModalEl = document.getElementById('addHolidayYearModal')
    addHolidayYearModalEl.addEventListener('hide.tw.modal', function(event) {
        ...
        $('#addHolidayYearModal .modal-footer input[type="checkbox"]').prop('checked', true);
    });
    ...
    editRoleModalEl.addEventListener('hide.tw.modal', function(event) {
        ...
        $('#editHolidayYearModal .modal-footer input[type="checkbox"]').prop('checked', false);
    ...
    $('#addHolidayYearForm').on('submit', function(e){
        ...
        document.querySelector('#saveHY svg').style.cssText = 'display: inline-block;';
        ...
        axios({
            ...
            document.querySelector('#saveHY svg').style.cssText = 'display: none;';
            ...
            if (response.status == 200) {
                ...
                document.getElementById('successModal').addEventListener('shown.tw.modal', function(event){
                    ...
                    $('#successModal .successModalDesc').html('HR Holiday years successfully inserted.');
                });
            }  
            hrHolidayYearsListTable.init();             
        }).catch(error => {
            ...
            if (error.response) {
                if (error.response.status == 422) {
                    for (const [key, val] of Object.entries(error.response.data.errors)) {
                        $(`#addHolidayYearForm .${key}`).addClass('border-danger')
    ...
    $("#hrHolidayYearsListTable").on("click", ".edit_btn", function () {      
        ...
        axios({
            ...
            if (response.status == 200) {
                ...
                $('#editHolidayYearModal input[name="notice_period"]').val(dataset.notice_period ? dataset.notice_period : '');
                ...
                if(dataset.active == 1){
                    $('#editHolidayYearModal input[name="active"]').prop('checked', true);
                }else{
                    $('#editHolidayYearModal input[name="active"]').prop('checked', false);
                }
    ...
    $('#editHolidayYearForm').on('submit', function(e){
        ...
        document.querySelector('#updateHY').setAttribute('disabled', 'disabled');
        document.querySelector('#updateHY svg').style.cssText = 'display: inline-block;';
        ...
        axios({
            ...
            document.querySelector('#updateHY svg').style.cssText = 'display: none;';
