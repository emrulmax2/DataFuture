...
var table = (function () {
    var _tableGen = function () {
        ...
        let tableContent = new Tabulator("#modulelevelTableId", {
            ajaxURL: route("modulelevels.list"),
        ...
        $("#tabulator-export-xlsx").on("click", function (event) {
            ...
            tableContent.download("xlsx", "data.xlsx", {
                sheetName: "Module Level Details",
            });
        });
...
(function () {
    ...
    if ($("#modulelevelTableId").length) {
        ...
        $("#modulelevelTableId").on("click", ".edit_btn", function () {      
            let $editBtn = $(this);
            let editId = $editBtn.attr("data-id");

            axios({
                method: "get",
                url: route("modulelevels.edit", editId),
                headers: {"X-CSRF-TOKEN": $('meta[name="csrf-token"]').attr("content")},
            }).then((response) => {
                if (response.status == 200) {
        ...
        $("#editForm").on("submit", function (e) {
            ...
            axios({
                method: "post",
                url: route("modulelevels.update", editId),
                data: form_data,
                headers: {
        ...
        $('#confirmModal .agreeWith').on('click', function(){
            ...
            if(action == 'DELETE'){
                axios({
                    method: 'delete',
                    url: route('modulelevels.destory', recordID),
                    headers: {'X-CSRF-TOKEN' :  $('meta[name="csrf-token"]').attr('content')},
                }).then(response => {
                    if (response.status == 200) {
                ...
                axios({
                    method: 'post',
                    url: route('modulelevels.restore', recordID),
                    headers: {'X-CSRF-TOKEN' :  $('meta[name="csrf-token"]').attr('content')},
                }).then(response => {
                    if (response.status == 200) {
                        $('#confirmModal button').removeAttr('disabled');
        ...
        // Delete Course
        $('#modulelevelTableId').on('click', '.delete_btn', function(){
            const confModal = tailwind.Modal.getOrCreateInstance(document.querySelector("#confirmModal"));
            document.getElementById('confirmModal').addEventListener('hidden.tw.modal', function(event){
                $('#confirmModal .agreeWith').attr('data-id', '0');
                $('#confirmModal .agreeWith').attr('data-action', 'none');
        ...
        // Restore Course
        $('#modulelevelTableId').on('click', '.restore_btn', function(){
            const confModal = tailwind.Modal.getOrCreateInstance(document.querySelector("#confirmModal"));
            document.getElementById('confirmModal').addEventListener('hidden.tw.modal', function(event){
                $('#confirmModal .agreeWith').attr('data-id', '0');
                $('#confirmModal .agreeWith').attr('data-action', 'none');
