...
(function () {
    ...
    function resetGroupSearch() {
        ...
        intake_semester.refreshOptions(false);
    ...
    $('#studentGroupSearchForm').on('submit', function (event) {
        ...
        axios({
            method: 'post',
            url: route('report.student.progress.total'), 
            data: form_data,
            headers: {
            ...
            .then((response) => {
                ...
                if (response.status == 200) {
                    ...
                    const jsonData = JSON.stringify(response.data.search_criteria);
                    localStorage.setItem(
                        'searchedCriteria25',
                        jsonData
                    );
        ...
        if (studentIds.length > 0) {
            ...
            axios({
                ...
                .then((response) => {
                    ...
                    link.setAttribute(
                        'download',
                        'student_progress_monitoring_report.xlsx'
                    );
