...
var table = (function () {
    var _tableGen = function () {
        ...
        let tableContent = new Tabulator("#semesterTableId", {
            ajaxURL: route("semester.list"),
...
(function () {
    ...
    if ($("#semesterTableId").length) {
        ...
        $("#semesterTableId").on("click", ".edit_btn", function () {      
            let $editBtn = $(this);
            let editId = $editBtn.attr("data-id");

            axios({
                method: "get",
                url: route("semester.edit", editId),
                headers: {
        ...
        $("#editForm").on("submit", function (e) {
            ...
            axios({
                method: "post",
                url: route("semester.update", editId),
                data: form_data,
                headers: {
                    "X-CSRF-TOKEN": $('meta[name="csrf-token"]').attr("content"),
                },
        ...
        $('#confirmModal .agreeWith').on('click', function(){
            ...
            if(action == 'DELETE'){
                ...
                axios({
                    method: 'post',
                    url: route('semester.restore', recordID),
                    headers: {'X-CSRF-TOKEN' :  $('meta[name="csrf-token"]').attr('content')},
                }).then(response => {
                    if (response.status == 200) {
                        $('#confirmModal button').removeAttr('disabled');
        ...
        // Delete Course
        $('#semesterTableId').on('click', '.delete_btn', function(){
            const confModal = tailwind.Modal.getOrCreateInstance(document.querySelector("#confirmModal"));
            document.getElementById('confirmModal').addEventListener('hidden.tw.modal', function(event){
                $('#confirmModal .agreeWith').attr('data-id', '0');
                $('#confirmModal .agreeWith').attr('data-action', 'none');
        ...
        // Restore Course
        $('#semesterTableId').on('click', '.restore_btn', function(){
            const confModal = tailwind.Modal.getOrCreateInstance(document.querySelector("#confirmModal"));
            document.getElementById('confirmModal').addEventListener('hidden.tw.modal', function(event){
                $('#confirmModal .agreeWith').attr('data-id', '0');
                $('#confirmModal .agreeWith').attr('data-action', 'none');
