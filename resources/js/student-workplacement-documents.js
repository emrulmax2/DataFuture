...
const studentWorkPlacementDocumentsTable = (function () {
    var _tableGen = function () {

        let studentId = $("#studentWorkPlacementDocumentsTable").attr('data-student') != "" ? $("#studentWorkPlacementDocumentsTable").attr('data-student') : "0";
        let rowId = $("#studentWorkPlacementDocumentsTable").attr('data-row') != "" ? $("#studentWorkPlacementDocumentsTable").attr('data-row') : "0";
        let queryStr = $("#query-UP").val() != "" ? $("#query-UP").val() : "";
        let status = $("#status-UP").val() != "" ? $("#status-UP").val() : "1";

        let tableContent = new Tabulator("#studentWorkPlacementDocumentsTable", {
            ajaxURL: route("student.workplacement.documents.list"),
            ajaxParams: { studentId: studentId, queryStr : queryStr, status : status, rowId : rowId},
            ajaxFiltering: true,
            ajaxSorting: true,
            printAsHtml: true,
            printStyled: true,
            ...
            renderComplete() {
                ...
                const actionColumn = tableContent.getColumn("id");
                if (actionColumn) {
...
export default studentWorkPlacementDocumentsTable;



(function(){
    if ($("#studentWorkPlacementDocumentsTable").length) {
        // Init Table
    ...
    uploadDocumentModalEl.addEventListener('hide.tw.modal', function(event) {
        $('#uploadDocumentModal input[name="student_workplacement_id"]').val('0');
...
//             if (instance.element === $("#uploadDocumentForm")[0]) {
//                 instance.destroy();
//             }
//         });
//     }
...
//     };
    ...
    //         $('#uploadDocumentModal input[name="student_workplacement_id"]').val(documentSettingId);
    ...
    $('#studentWorkPlacementDocumentsTable').on('click', '.delete_btn', function(e){
        e.preventDefault();
        var $btn = $(this);
    ...
    $('#studentWorkPlacementDocumentsTable').on('click', '.restore_btn', function(e){
        e.preventDefault();
        var $btn = $(this);
        var uploadId = $btn.attr('data-id');
    ...
    $('#confirmModal .agreeWith').on('click', function(e){
        ...
        if(action == 'DELETEDOC'){
            axios({
                method: 'delete',
                url: route('student.workplacement.documents.destory'),
            ...
            axios({
                method: 'post',
                url: route('student.workplacement.documents.restore'),
                data: {student : student, recordid : recordid},
                headers: {'X-CSRF-TOKEN' :  $('meta[name="csrf-token"]').attr('content')},
            }).then(response => {
                if (response.status == 200) {
    ...
    $('#studentWorkPlacementDocumentsTable').on('click', '.downloadDoc', function(e){
        e.preventDefault();
        var $theLink = $(this);
        ...
        axios({
            method: "post",
            url: route('student.workplacement.documents.download'), 
            data: {row_id : row_id},
            headers: {'X-CSRF-TOKEN' :  $('meta[name="csrf-token"]').attr('content')},
        }).then(response => {
            if (response.status == 200){
