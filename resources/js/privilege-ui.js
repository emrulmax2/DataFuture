...
/**
 * Progressive enhancement for the privilege forms. It reads plain card markup
 * (`.intro-y.box` sections holding `.form-check.form-switch` rows) and builds the
 * group rail, counters, enable-all buttons, search filter, collapsing and the
 * print report on top of it.
 *
 * Shared by the legacy privilege page and the template-driven one, whose markup
 * is swapped over AJAX — so init() must be safe to call again on fresh markup.
 */

// Bound once, but dispatched through a ref so a re-init's handlers replace the
// previous ones instead of the listener holding a stale closure over old cards.
let printListenersBound = false;
const activePrint = { begin: null, end: null };

export function initPrivilegeUi(formSelector = "#employeePrivilegeForm") {
    ...
    // Re-init runs against freshly injected cards, so drop anything the previous
    // pass left behind rather than stacking duplicate rail links.
    railList.replaceChildren();
    sections.querySelectorAll(".ep-privilege-empty").forEach((node) => node.remove());
    ...
    const createPrintGroupTitle = (label) => {
        return label ? createPrintElement("div", "ep-privilege-print-group-title", label) : null;
    };
    ...
    const appendPrintFieldRows = (sourceBlock, printBlock) => {
        ...
        Array.from(sourceBlock.querySelectorAll("input[type=\"text\"]")).forEach((input) => {
            const field = createPrintFieldRow(formatControlLabel(input), input.value.trim());
    ...
    // Toolbar controls live outside the swapped markup, so rebind rather than
    // stack a second listener on every template reload.
    $(expandAll).off("click.privilegeUi").on("click.privilegeUi", () => {
        cardData.forEach((item) => setCardExpanded(item, true));
    });

    $(collapseAll).off("click.privilegeUi").on("click.privilegeUi", () => {
        cardData.forEach((item) => setCardExpanded(item, false));
    });

    $(printButton).off("click.privilegeUi").on("click.privilegeUi", printPrivileges);
    $(searchInput).off("input.privilegeUi").on("input.privilegeUi", filterCards);

    $(form).off("change.privilegeUi").on("change.privilegeUi", "input[type=\"checkbox\"], select", function () {
        window.setTimeout(updateCounts, 0);
    });

    activePrint.begin = beginPrivilegePrint;
    activePrint.end = endPrivilegePrint;

    if (!printListenersBound) {
        window.addEventListener("beforeprint", () => activePrint.begin?.());
        window.addEventListener("afterprint", () => activePrint.end?.());
