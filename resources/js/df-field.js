...
var settingsListTable = (function () {
    var _tableGen = function () {
        ...
        let tableContent = new Tabulator("#settingsListTable", {
            ...
            ajaxParams: { querystr: querystr, status: status, category: category },
            ajaxFiltering: true,
            ajaxSorting: true,
            printAsHtml: true,
            printStyled: true,
...
(function () {
    ...
    if ($("#settingsListTable").length) {
        ...
        addSettingsModalEl.addEventListener('hide.tw.modal', function(event) {
            ...
            $('#addSettingsModal .modal-body textarea').val('');
        });
        ...
        editSettingsModalEl.addEventListener('hide.tw.modal', function(event) {
            ...
            $('#editSettingsModal .modal-body textarea').val('');
        });


        if($('.inputUppercase').length > 0){
            ...
            axios({
                ...
                if (response.status == 200) {
                    ...
                    document.getElementById("successModal").addEventListener("shown.tw.modal", function (event) {
                            ...
                            $("#successModal .successModalDesc").html('Datafuture field category Successfully inserted.');
                    });     
                }
                settingsListTable.init();
            }).catch(error => {
            ...
            axios({
                ...
                .then((response) => {
                    if (response.status == 200) {
                        ...
                        $('#editSettingsModal select[name="datafuture_field_category_id"]').val(dataset.datafuture_field_category_id ? dataset.datafuture_field_category_id : '');
                        $('#editSettingsModal select[name="type"]').val(dataset.type ? dataset.type : '');
                        $('#editSettingsModal textarea[name="description"]').val(dataset.description ? dataset.description : '');
        ...
        $("#editSettingsForm").on("submit", function (e) {
            ...
            axios({
                ...
                if (response.status == 200) {
                    ...
                    document.getElementById("successModal").addEventListener("shown.tw.modal", function (event) {
                        $("#successModal .successModalTitle").html("Congratulations!");
                        $("#successModal .successModalDesc").html('Datafuture field category successfully updated.');
                    });
                }
                settingsListTable.init();
            }).catch((error) => {
