                ...
                if (error.response) {
                    if (error.response.status == 422) {
                        for (const [key, val] of Object.entries(error.response.data.errors)) {
                            $(`#assign .${key}`).addClass('border-danger')
                            $(`#assign  .error-${key}`).html(val)
                        }
                        $('#assign #name').val('');
                    } else {
                        console.log('error');
