...
class CustomExpectedResultCollectionExport implements FromCollection, WithHeadings, WithStyles, WithColumnWidths
{
    protected $data;
    protected $headers;
    ...
    public function styles(Worksheet $sheet)
    {
        ...
        foreach ($this->data as $rowIndex => $row) {
            foreach ($this->moduleList as $colIndex => $module) {
                $cellValue = $row[$colIndex + 8]; // Adjust index based on your data structure
                if (isset($cellValue) && $cellValue!="") {
                    $rgbColor = ($cellValue == 'No') ? 'FF797A' : 'fed7aa'; // Change color as needed
                    $cellCoordinate = \PhpOffice\PhpSpreadsheet\Cell\Coordinate::stringFromColumnIndex($colIndex + 9) . ($rowIndex + 2); // Adjust index based on your data structure
