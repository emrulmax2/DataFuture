...
class ResultSubmissionImport implements ToModel, WithHeadingRow
{
    ...
    */
    public function model(array $row)
    {
        ...
        $assessmentPlan = AssessmentPlan::where('course_module_base_assesment_id',$this->courseMoudleBaseAssessment)->where('plan_id',$this->plan->id)->whereNull('is_it_final')->get()->first();
        if(!$assessmentPlan){
        ...
        $studentUserId = StudentUser::where('email', $row["email"])->get()->first();
            if($studentUserId!=null){
                ...
                return new ResultSubmission([
                    ...
                    'upload_user_type' => 'personal_tutor',
                    'created_by' => Auth::id()
                ]);
            }
