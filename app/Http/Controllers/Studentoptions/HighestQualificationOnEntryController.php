...
class HighestQualificationOnEntryController extends Controller
{
    public function list(Request $request){
        ...
        $query = HighestQualificationOnEntry::orderByRaw(implode(',', $sorts));
        if(!empty($queryStr)):
    ...
    public function store(OptionValueRequest $request){
        $data = HighestQualificationOnEntry::create([
            'name'=> $request->name,
    ...
    public function edit($id){
        $data = HighestQualificationOnEntry::find($id);
    ...
    public function update(OptionValueRequest $request){      
        $data = HighestQualificationOnEntry::where('id', $request->id)->update([
            'name'=> $request->name,
            'is_hesa'=> (isset($request->is_hesa) ? $request->is_hesa : 0),
            'hesa_code'=> (isset($request->is_hesa) && $request->is_hesa == 1 && !empty($request->hesa_code) ? $request->hesa_code : null),
            'is_df'=> (isset($request->is_df) ? $request->is_df : 0),
    ...
    public function destroy($id){
        $data = HighestQualificationOnEntry::find($id)->delete();
        return response()->json($data);
    }

    public function restore($id) {
        $data = HighestQualificationOnEntry::where('id', $id)->withTrashed()->restore();

        response()->json($data);
    }

    public function updateStatus($id){
        $title = HighestQualificationOnEntry::find($id);
        $active = (isset($title->active) && $title->active == 1 ? 0 : 1);

        HighestQualificationOnEntry::where('id', $id)->update([
            'active'=> $active,
            'updated_by' => auth()->user()->id
        ]);
