...
class EligibilityReportController extends Controller
{
    ...
    public function visaList(Request $request){
        ...
        if(!empty($Query)):
            ...
            foreach($Query as $list):
                $permitExpDate = strtotime($list->eligibilities->workpermit_expire);
    ...
    public function passportList(Request $request){
        ...
        if(!empty($Query)):
            $i = 1;
            foreach($Query as $list):
                $permitExpDate = strtotime($list->eligibilities->doc_expire);
                $today = strtotime(date('Y-m-d'));
                $secs =  $permitExpDate-$today;
    ...
    public function generatePDF(Request $request)
    {
        ...
        foreach($items as $item) {
            ...
            $workPermitExpDate = strtotime($eligibilities->workpermit_expire);
            $today = strtotime(date('Y-m-d'));
            ...
            $docPermitExpDate = strtotime($eligibilities->doc_expire);
            $today = strtotime(date('Y-m-d'));
        ...
        $pdf = PDF::loadView('pages.hr.portal.reports.pdf.eligibilitypdf',compact('dataList'));
        return $pdf->download('Eligibility Expiry Report.pdf');
