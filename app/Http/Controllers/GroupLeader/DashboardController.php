...
class DashboardController extends Controller
{
    ...
    /**
     * The grade that means nothing was handed in. The submission rate card
     * counts every other grade as a submission: a referral or a fail is work
     * that came in, just not work that passed.
     */
    private const NOT_SUBMITTED_GRADES = ['A'];
     ...
     */
    private function studentSubmissions(array $planIds, array $studentIds, ?array $gradeIds = null): array
    {
        ...
        // The grades that count; completed ones unless the caller says otherwise.
        $gradeIds = $gradeIds ?? Grade::whereIn('code', self::COMPLETED_GRADES)->pluck('id')->toArray();
     ...
     * whether it passed — that is the Subs figure's question.
     *
     * The rate is the group's own ratio — every module owed by every student
     * over every one handed in — not the average of the students'
     * percentages, which would let a student on one module weigh as heavily as
     * one on five.
     *
     * A module counts once whatever it took to get there: `studentSubmissions()`
     * folds retakes and re-marks into the module, so an Absent first attempt
     * followed by a referral is one module handed in.
     */
    private function termSubmissionRate(array $planIds, array $studentIds): array
    {
        ...
        // Straight off the table rather than the model: a grade retired since
        // (Fail) still marks work that was handed in at the time. The blank
        // grade row is not a result anyone entered.
        $handedIn = DB::table('grades')
            ->whereNotNull('code')->where('code', '!=', '')
            ->whereNotIn('code', self::NOT_SUBMITTED_GRADES)
            ->pluck('id')->map(fn ($id) => (int) $id)->toArray();

        $subs = $this->studentSubmissions($this->theoryModulePlanIds($planIds), $studentIds, $handedIn);
