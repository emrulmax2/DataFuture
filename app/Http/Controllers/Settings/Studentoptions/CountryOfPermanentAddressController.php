...
class CountryOfPermanentAddressController extends Controller
{
    public function list(Request $request){
        ...
        $query = CountryOfPermanentAddress::orderByRaw(implode(',', $sorts));
        if(!empty($queryStr)):
    ...
    public function store(OptionValueRequest $request){
        $data = CountryOfPermanentAddress::create([
            'name'=> $request->name,
    ...
    public function edit($id){
        $data = CountryOfPermanentAddress::find($id);
    ...
    public function update(OptionValueRequest $request){      
        $data = CountryOfPermanentAddress::where('id', $request->id)->update([
            'name'=> $request->name,
            'is_hesa'=> (isset($request->is_hesa) ? $request->is_hesa : 0),
            'hesa_code'=> (isset($request->is_hesa) && $request->is_hesa == 1 && !empty($request->hesa_code) ? $request->hesa_code : null),
            'is_df'=> (isset($request->is_df) ? $request->is_df : 0),
    ...
    public function destroy($id){
        $data = CountryOfPermanentAddress::find($id)->delete();
        return response()->json($data);
    }

    public function restore($id) {
        $data = CountryOfPermanentAddress::where('id', $id)->withTrashed()->restore();

        response()->json($data);
    }

    public function updateStatus($id){
        $title = CountryOfPermanentAddress::find($id);
        $active = (isset($title->active) && $title->active == 1 ? 0 : 1);

        CountryOfPermanentAddress::where('id', $id)->update([
            'active'=> $active,
            'updated_by' => auth()->user()->id
        ]);
